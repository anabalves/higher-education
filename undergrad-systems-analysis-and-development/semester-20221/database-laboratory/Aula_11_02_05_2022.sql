...
Agora crie uma trigger que ao inserir um PreReq ela garanta que a  Disciplina exista. 
Tambem caso a Disciplina seja excluida ela valide se existem PreReq, caso exista PreReq nao permitir a exclusao.
*/

USE AULA_11_02_05_2022;

DROP TRIGGER IF EXISTS TGR_INSERT_INTEGRITY_TB_PRE_REQ;
DROP TRIGGER IF EXISTS TGR_DELETE_INTEGRITY_TB_DISCIPLINA;

DELIMITER $$
CREATE TRIGGER TGR_INSERT_INTEGRITY_TB_PRE_REQ 
BEFORE INSERT
ON TB_PRE_REQ
FOR EACH ROW
BEGIN
	IF ((SELECT 
		...
		FROM 
			TB_DISCIPLINA 
		WHERE 
			NEW.COD_DEPTO = COD_DEPTO 
		AND 
			NEW.NUM_DISC = NUM_DISC) IS NULL) 
	THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Nao pode inserir, a disciplina nao existe.';
	END IF;
END $$

CREATE TRIGGER TGR_DELETE_INTEGRITY_TB_DISCIPLINA
BEFORE DELETE
ON TB_DISCIPLINA
FOR EACH ROW
BEGIN
	IF ((SELECT 
		...
		FROM 
			TB_PRE_REQ 
		WHERE 
			OLD.COD_DEPTO = COD_DEPTO 
		AND 
			OLD.NUM_DISC = NUM_DISC) IS NULL) 
	THEN
		SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Nao se pode apagar uma Disciplina com Pre-requisito';
	END IF;
END $$
DELIMITER ;

INSERT INTO TB_PRE_REQ(COD_DEPTO, NUM_DISC, COD_DEPTO_PRE_REQ, NUM_DISC_PRE_REQ) VALUES ('INF01', 6, 'INF01', 3);
INSERT INTO TB_PRE_REQ(COD_DEPTO, NUM_DISC, COD_DEPTO_PRE_REQ, NUM_DISC_PRE_REQ) VALUES ('MAT01', 4, 'INF01', 2);

DELETE FROM TB_DISCIPLINA WHERE NUM_DISC = 2;
DELETE FROM TB_DISCIPLINA WHERE NUM_DISC = 1;

/* 
DROP DATABASE IF EXISTS AULA_11_02_05_2022;

CREATE DATABASE AULA_11_02_05_2022;
